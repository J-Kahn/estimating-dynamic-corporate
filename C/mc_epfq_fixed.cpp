...
int main(int argc, char **argv){
  ...
  int thr_num  = 14;
	...
	matrix moms = readcsv("../Results/MC/trial_data_rr_fixed_quad.csv", 5000, 6 * 3);
	matrix vmoms = readcsv("../Results/MC/trial_data_rr_fixed_wquad.csv", 5000, 6 * 6 * 3 * 3);
	...
	matrix randus(nobsper * nfirms + 50 * nfirms + 3 * nfirms, 10);
	int seedn = 1158;
	srand(seedn);
	randus.randu();
...
string res, filename, filername, resr;
	...
	filername = "../Results/MC/Trials/summary_quad_fixed_rr10" + resr;
	...
	for(int run = id * nper + start; run < (id +1) * nper + start; run++){
       ...
       res1.reset();
	    matrix results = simann(init_points, false, 0.85, 1e-6, 20, 5, -4, 150000, minb, maxb, cb, -4, 100, 0.005,
	                       vm1, dat1, res1, m, vcvm, mn, randus, filename, "../Results/MC/Trials/", objec_epf_quad, f, f_0, thr_num, 5);
