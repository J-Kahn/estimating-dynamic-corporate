...
int main(int argc, char **argv){
  ...
  int thr_num  = 14;
	...
	matrix moms = readcsv("../Results/MC/trial_data_rr_mom_cv.csv", 1000, 11);
	matrix vmoms = readcsv("../Results/MC/trial_data_rr_wmom_cv.csv", 1000, 11 * 11);
...
string res, filename, filername, resr;
	...
	filername = "../Results/MC/Trials/summary_mom_cv_rr15" + resr;
  ...
  matrix var, var2, var3, jstat, jstat2, mn2, m, mn(moms.ncols());
	...
	// Run estimation
      ...
      res1.reset();
	    matrix results = simann(init_points, false, 0.85, 1e-6, 20, 5, 4, 150000, minb, maxb, cb, -4, 100, 0.005,
	                       vm1, dat1, res1, m, vcvm, mn, randus, filename, "../Results/MC/Trials/", objec_mom_cv, f, f_0, thr_num, 4);
    ...
    double fold = f;
      ...
      jacobians(results, res1, dat1, randus, mom_tm_cv_est, epf_quad_est, 10, thr_num, 4, 4, jac, jac2, 1e-6, 4);
