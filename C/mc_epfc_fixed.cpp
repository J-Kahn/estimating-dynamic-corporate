...
int main(int argc, char **argv){
  ...
  int thr_num  = 14;
	...
	matrix init_points = readcsv("../Results/Estim/par_quad_toni_rr3_fixed.csv",8,1);
	...
	matrix moms = readcsv("../Results/MC/trial_data_rr_fixed_cube.csv", 1000, 10 * 3);
	matrix vmoms = readcsv("../Results/MC/trial_data_rr_fixed_wcube.csv", 1000, 10 * 10 * 3 * 3);
	...
	matrix randus(nobsper * nfirms + 3 * nfirms + 50 * nfirms, 10);
	int seedn = 1158;
	srand(seedn);
	randus.randu();
  ...
  dynprob res1 = dynprob(24, 64, 121, 15);
  res1.delp = 8;
	// Initialize interest rate
	res1.pars[0]=1/(1+1.1*0.02);
...
string res, filename, filername, resr;
	...
	filername = "../Results/MC/Trials/summary_cube_fixed_rr" + resr;
	...
	for(int run = id * nper + start; run < (id +1) * nper + start; run++){
       ...
       res1.reset();
	    matrix results = simann(init_points, false, 0.85, 1e-6, 20, 5, -4, 150000, minb, maxb, cb, -4, 100, 0.005,
	                       vm1, dat1, res1, m, vcvm, mn, randus, filename, "../Results/MC/Trials/", objec_epf_cube, f, f_0, thr_num, 5);
     ...
     double fold = f;
      ...
      jacobians(results, res1, dat1, randus, mom_tm_est, epf_cube_est, 10, thr_num, 4, 4, jac, jac2, 1e-6, 4);
