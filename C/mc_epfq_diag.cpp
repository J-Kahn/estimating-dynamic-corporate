...
int main(int argc, char **argv){
  ...
  int thr_num = 14;
	...
	matrix moms = readcsv("../Results/MC/trial_data_rr_quad.csv", 5000, 6 * 3);
	matrix vmoms = readcsv("../Results/MC/trial_data_rr_wquad_unclust_diag.csv", 5000, 6 * 6 * 3 * 3);
...
string res, filename, filername, resr;
	...
	filername = "../Results/MC/Trials/summary_quad_diag_rr10" + resr;
  ...
  matrix var, var2, var3, jstat, jstat2, mn2, m, mn;
	...
	for(int run = id * nper + start; run < (id +1) * nper + start; run++){
      ...
      // Do actual estimation
	    matrix results = simann(init_points, false, 0.85, 1e-6, 20, 5, 4, 150000, minb, maxb, cb, 2, 100, 0.005,
	                       vm1, dat1, res1, m, vcvm, mn, randus, filename, "../Results/MC/Trials/", objec_epf_quad, f, f_0, thr_num, 4);
      matrix jac, jac2, jac5, jac52;
      res1.wipe_long();
      matrix res_old = results;
      double fold = f;

     // Commented out options for Nelder-Mead post simulated annealing.
