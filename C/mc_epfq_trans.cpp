...
int main(int argc, char **argv){
  ...
  int thr_num  = 14;
	...
	matrix moms = readcsv("../Results/MC/trial_data_rr_quad_trans.csv", 1000, 6 * 3);
	matrix vmoms = readcsv("../Results/MC/trial_data_rr_wquad_trans.csv", 1000, 6 * 6 * 3 * 3);
...
string res, filename, filername, resr;
	...
	filername = "../Results/MC/Trials/summary_quad_trans_rr10" + resr;
	...
	for(int run = id * nper + start; run < (id +1) * nper + start; run++){
       ...
       res1.reset();
	    matrix results = simann(init_points, false, 0.85, 1e-6, 20, 5, 4, 150000, minb, maxb, cb, -4, 100, 0.005,
	                       vm1, dat1, res1, m, vcvm, mn, randus, filename, "../Results/MC/Trials/", objec_epf_quad_trans, f, f_0, thr_num, 4);
     ...
     double fold = f;
      ...
      jacobians(results, res1, dat1, randus, mom_tm_est, epf_quad_trans_est, 10, thr_num, 4, 4, jac, jac2, 1e-6, 4);
