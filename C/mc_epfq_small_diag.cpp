...
int main(int argc, char **argv){
  ...
  int thr_num = 14;
	...
	matrix moms = readcsv("../Results/MC/trial_data_rr_small_quad.csv", 5000, 6 * 3);
	matrix vmoms = readcsv("../Results/MC/trial_data_rr_small_wquad_unclust_diag.csv", 5000, 6 * 6 * 3 * 3);
...
string res, filename, filername, resr;
	...
	filername = "../Results/MC/Trials/summary_quad_rr10_small_diag" + resr;
  ...
  matrix var, var2, var3, jstat, jstat2, mn2, m, mn;
	...
	for(int run = id * nper + start; run < (id +1) * nper + start; run++){
  		...
  		convert << id << "_" << run;
  		res = convert.str();
  		filename = "quad_rr10_small_diag" + res;
